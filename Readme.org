...
- API for anaphoric macros is changed.
  ...
  Please use =(! it)= or =(!it)= instead of =(it)=. You can also write
  more complex expressions inside =(! ...)=, for example: =(! (car
  it))=.
...
script no slower. =(!it)= is a shorthand for =(! it)=.

Other anaphoric macros provided by setup.el are: =!if=, =!when=,
=!cond=, =!unless=, =!foreach=. More complex expressions can also be
passed to =!=:
